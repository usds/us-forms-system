...
var _concat2 = require('lodash/fp/concat');
...
exports.filterInactivePages = filterInactivePages;
...
function getActiveProperties(activePages) {
  var allProperties = [];
  activePages.map(function (page) {
    if (page.schema) {
      allProperties = (0, _uniq3.default)((0, _concat3.default)(Object.keys(page.schema.properties), allProperties));
    }
...
function filterInactivePages(inactivePages, activePages, form) {
  ...
  return inactivePages.reduce(function (formData, page) {
    return Object.keys(page.schema.properties).reduce(function (currentData, prop) {
      if (!activeProperties.includes(prop)) {
        newData = (0, _unset3.default)(prop, currentData);
      }
      return newData;
    }, formData);
  }, form.data);
...
function transformForSubmit(formConfig, form) {
  ...
  var activePages = getActivePages(createFormPageList(formConfig), form.data);
  var inactivePages = getInactivePages(createFormPageList(formConfig), form.data);
  var withoutInactivePages = filterInactivePages(inactivePages, activePages, form);
